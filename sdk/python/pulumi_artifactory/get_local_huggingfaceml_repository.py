...
__all__ = [
    'GetLocalHuggingfacemlRepositoryResult',
    'AwaitableGetLocalHuggingfacemlRepositoryResult',
    'get_local_huggingfaceml_repository',
    'get_local_huggingfaceml_repository_output',
]

@pulumi.output_type
class GetLocalHuggingfacemlRepositoryResult:
    """
...
class AwaitableGetLocalHuggingfacemlRepositoryResult(GetLocalHuggingfacemlRepositoryResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetLocalHuggingfacemlRepositoryResult(
...
def get_local_huggingfaceml_repository(archive_browsing_enabled: Optional[builtins.bool] = None,
    ...
    :param builtins.bool archive_browsing_enabled: When set, you may view content such as HTML or Javadoc files directly from Artifactory.
           This may not be safe and therefore requires strict content moderation to prevent malicious users from uploading content that may compromise security (e.g., cross-site scripting attacks).
    :param builtins.bool blacked_out: When set, the repository does not participate in artifact resolution and new artifacts cannot be deployed.
    :param builtins.bool cdn_redirect: When set, download requests to this repository will redirect the client to download the artifact directly from AWS CloudFront. Available in Enterprise+ and Edge licenses only. Default value is 'false'
    :param builtins.str description: Public description.
    :param builtins.bool download_direct: When set, download requests to this repository will redirect the client to download the artifact directly from the cloud storage provider. Available in Enterprise+ and Edge licenses only.
    :param builtins.str excludes_pattern: List of artifact patterns to exclude when evaluating artifact requests, in the form of x/y/**/z/*.By default no artifacts are excluded.
    :param builtins.str includes_pattern: List of comma-separated artifact patterns to include when evaluating artifact requests in the form of x/y/**/z/*. When used, only artifacts matching one of the include patterns are served. By default, all artifacts are included (**/*).
    :param builtins.str key: A mandatory identifier for the repository that must be unique. Must be 3 - 10 lowercase alphanumeric and hyphen characters. It cannot begin with a number or contain spaces or special characters.
    :param builtins.str notes: Internal description.
    :param builtins.bool priority_resolution: Setting repositories with priority will cause metadata to be merged only from repositories set with this field
    :param builtins.str project_key: Project key for assigning this repository to. Must be 2 - 20 lowercase alphanumeric and hyphen characters. When assigning repository to a project, repository key must be prefixed with project key, separated by a dash.
    :param Sequence[builtins.str] property_sets: List of property set name
    :param builtins.str repo_layout_ref: Repository layout key for the local repository
    :param builtins.bool xray_index: Enable Indexing In Xray. Repository will be indexed with the default retention period. You will be able to change it via Xray settings.
    """
    __args__ = dict()
    ...
    __ret__ = pulumi.runtime.invoke('artifactory:index/getLocalHuggingfacemlRepository:getLocalHuggingfacemlRepository', __args__, opts=opts, typ=GetLocalHuggingfacemlRepositoryResult).value

    return AwaitableGetLocalHuggingfacemlRepositoryResult(
...
def get_local_huggingfaceml_repository_output(archive_browsing_enabled: Optional[pulumi.Input[Optional[builtins.bool]]] = None,
