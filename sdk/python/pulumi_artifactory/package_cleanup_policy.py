...
class PackageCleanupPolicy(pulumi.CustomResource):
    ...
    def __init__(__self__,
        ...
        **Important Notes:**
        ...
        You may combine **time-based** fields with **`included_properties`** in the same policy. On the Artifactory platform, matching both time and property criteria uses **AND** semantics from **Artifactory 7.129** onward. **`keep_last_n_versions`** cannot be used together with time-based or properties-based conditions.
        ...
        2. **Version-based exclusivity**: `keep_last_n_versions` cannot be combined with time-based conditions or with `included_properties`. Time-based and properties-based conditions **may** be combined (AND semantics on Artifactory **7.129+**).
        ...
        - Combining time-based conditions with `included_properties` in one policy (AND semantics) requires Artifactory **7.129** or later.
