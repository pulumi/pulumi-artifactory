...
class LdapSettingArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[bool] allow_user_to_access_profile: Auto created users will have access to their profile page and will be able to perform actions such as generating an API
               key. Default value is "false".
        :param pulumi.Input[bool] auto_create_user: When set, users are automatically created when using LDAP. Otherwise, users are transient and associated with auto-join
               groups defined in Artifactory. Default value is "true".
        :param pulumi.Input[str] email_attribute: An attribute that can be used to map a user's email address to a user created automatically in Artifactory. Default
               value is "mail".
        ...
        :param pulumi.Input[bool] paging_support_enabled: When set, supports paging results for the LDAP server. This feature requires that the LDAP server supports a
               PagedResultsControl configuration. Default value is "true".
        :param pulumi.Input[str] search_base: A context name to search in relative to the base DN of the LDAP URL. For example, 'ou=users' With the LDAP Group Add-on
               enabled, it is possible to enter multiple search base entries separated by a pipe ('|') character.
        :param pulumi.Input[str] search_filter: A filter expression used to search for the user DN used in LDAP authentication. This is an LDAP search filter (as
        ...
        :param pulumi.Input[str] user_dn_pattern: A DN pattern that can be used to log users directly in to LDAP. This pattern is used to create a DN string for 'direct'
