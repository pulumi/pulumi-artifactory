...
// Enable token (Bearer) based authentication. When set, use an access token as `password` (username may be empty) so Artifactory authenticates to the remote with a Bearer token instead of Basic auth. Default is `false` for most package types; OCI, Helm OCI, and Hugging Face remotes default to `true`.
func (o RemoteOpkgRepositoryOutput) EnableTokenAuthentication() pulumi.BoolOutput {
	return o.ApplyT(func(v *RemoteOpkgRepository) pulumi.BoolOutput { return v.EnableTokenAuthentication }).(pulumi.BoolOutput)
}
...
// **NOTE:** This field is write-only and its value will not be updated in state as part of read operations.
// Write-only equivalent of `password`. The value is used to authenticate against the remote registry but is **never stored in Terraform state or plan**. Requires Terraform 1.11 or later. Conflicts with `password`. Because write-only values are not tracked in state, use `passwordWoVersion` to signal when the secret has changed so it is re-sent to Artifactory.
func (o RemoteOpkgRepositoryOutput) PasswordWo() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *RemoteOpkgRepository) pulumi.StringPtrOutput { return v.PasswordWo }).(pulumi.StringPtrOutput)
}

// A version identifier for `passwordWo`. Change this value (for example, after rotating the secret) to trigger an update that re-sends the current `passwordWo` value to Artifactory. Only meaningful together with `passwordWo`.
func (o RemoteOpkgRepositoryOutput) PasswordWoVersion() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *RemoteOpkgRepository) pulumi.StringPtrOutput { return v.PasswordWoVersion }).(pulumi.StringPtrOutput)
}
